#!/usr/bin/env bash
# Run cheatah-plot's unit tests under Valgrind memcheck — a second memory checker alongside ASan (each
# catches things the other can miss). Uses the plain `debug` build, because Valgrind cannot run an
...
cmake --preset debug          >/tmp/cheatah_plot_vg_cfg.log   2>&1 || { tail -20 /tmp/cheatah_plot_vg_cfg.log;   exit 1; }
cmake --build --preset debug  >/tmp/cheatah_plot_vg_build.log 2>&1 || { tail -30 /tmp/cheatah_plot_vg_build.log; exit 1; }
