 ...
 * part — runs on the device through cheatah-gpu-linalg's context (see gpu_lane.hpp). On the
 * emulated-Metal lane the "device" is raster_cpu's own code registered as stand-ins, so those
 * bytes are IDENTICAL by construction; on Vulkan the kernels are the same expressions compiled
 * by slangc, identical outside sqrt/floor ULP noise inside the 1-pixel AA feather (the tests
 * bound it per channel).
 */
...
/// The smallest device buffer the raster step asks for: requests are floored here so an empty
/// draw list (a figure with no primitives) still binds a valid buffer.
 ...
 *
 * @param ctx The live context to dispatch on (see @ref detail::lane).
 ...
 * @alloc the returned framebuffer + the two qualified kernel names.
 ...
 */
inline std::vector<std::uint32_t> raster_gpu(gl::detail::Context& ctx,
    ...
    if (npix == 0) return {};
    using Buf = gl::detail::Buffer;
    const auto floored = [](std::size_t bytes) { return std::max(bytes, detail::kMinBufferBytes); };
    Buf* bufs[5] = {nullptr, nullptr, nullptr, nullptr, nullptr};   // the raster binding order
    try {
        bufs[0] = ctx.new_buffer(floored(prims.size() * sizeof(Prim)));
        if (!prims.empty()) ctx.upload(bufs[0], prims.data(), prims.size() * sizeof(Prim));
        bufs[1] = ctx.new_buffer(floored(bins.tile_offsets.size() * sizeof(std::uint32_t)));
        ctx.upload(bufs[1], bins.tile_offsets.data(),
                   bins.tile_offsets.size() * sizeof(std::uint32_t));
        bufs[2] = ctx.new_buffer(floored(bins.tile_prims.size() * sizeof(std::uint32_t)));
        ...
        bufs[4] = ctx.new_buffer(floored(sizeof(RasterParams)));
        ctx.upload(bufs[4], &params, sizeof(RasterParams));

        const std::string clear_kernel = detail::qualified(kClearKernel);
        const std::string raster_kernel = detail::qualified(kRasterKernel);
        Buf* clear_bufs[2] = {bufs[3], bufs[4]};
        ctx.dispatch_2d(clear_kernel.c_str(), clear_bufs, 2, params.width, params.height);
        ctx.dispatch_2d(raster_kernel.c_str(), bufs, 5, params.width, params.height);
...
inline Image render_gpu(cheatah::figure::Figure& fig) {
    ...
    std::vector<std::uint32_t> fb = raster_gpu(detail::lane(), dl, bins, params);
    return detail::pack_image(fb, w, h);
}

/**
 * Attempt the default GPU lane for one raster: honours `CHEATAH_PLOT_FORCE_CPU=1`, the cached
 * `gl::available()` probe, and a process-wide failure latch — the FIRST device failure prints
...
inline bool try_gpu(const std::vector<Prim>& prims, const TileBins& bins,
    ...
    if (gpu_failed || !gl::available()) return false;
    try {
        out_fb = raster_gpu(detail::lane(), prims, bins, params);
        return true;
