#!/usr/bin/env bash
# QA gate for cheatah-plot — the quality checks that must pass before a push. Invoked by the git
# pre-push hook (.githooks/pre-push) and runnable by hand. Exits non-zero to BLOCK the push.
#
#   1. Unit coverage (hard gate): clang source-based coverage of cheatah_plot_tests must be 100% lines
#      AND functions over plot/**/*.hpp; the README coverage table must be committed in sync.
#   2. Doc coverage (hard gate): 100% Javadoc on the hand-written plot C++ API (scripts/doc_coverage.sh).
#   3. Module sync + sidecars (hard gate): every generated plot/<mod>/<mod>.hpp still matches its `.purr`
#      (scripts/gen-headers.sh --check), and every module header verifies against its .sha512 sidecar —
#      the checksums that make `biome add cheatah-plot` resolvable on the extension path.
#   4. Configure + build (debug).
#   5. System tests (hard gate): every systests/test_*.purr compiles against the plot package via purrc
#      and runs under the cheatah runtime, finishing with `RESULT: PASS`.
#   5b. Biome-install sandbox (hard gate): a fresh `biome add cheatah-plot` user can compile + run it.
...
#
# The toolchain (purrc + cheatah runtime + the prebuilt stdlib archives the C++ tests link) is the
# sibling ../cheatah checkout; override with CHEATAH_DIR. Skips (discouraged; for fast local iteration):
...
if [ "${QA_GATE_SKIP_COVERAGE:-0}" = "1" ]; then
    ...
    bash scripts/coverage.sh update-readme >/tmp/cheatah_plot_coverage.log 2>&1 || { tail -30 /tmp/cheatah_plot_coverage.log; fail "coverage report"; }
    ...
    cat /tmp/cheatah_plot_coverage.log
    covnums=$(sed -n 's/.*lines [0-9.]*% (\([0-9]*\)\/\([0-9]*\)), functions [0-9.]*% (\([0-9]*\)\/\([0-9]*\)).*/\1 \2 \3 \4/p' /tmp/cheatah_plot_coverage.log)
...
# 2. Doc coverage: 100% Javadoc on the hand-written plot API (hard gate) --------------------------
...
# 3. Module sync + sidecars: the generated headers must match their `.purr`, and every module header
#    must verify against its .sha512 (what `biome add cheatah-plot` resolves on the extension path). ----
bold "Checking generated headers + module sidecars are in sync…"
bash scripts/gen-headers.sh --check || fail "a generated header drifted from its .purr — run scripts/gen-headers.sh, commit it, push again"
...
done < <(find plot -name '*.hpp.sha512')
# If the tree IS committed, also refuse a stale (uncommitted) sidecar so it can't be pushed out of sync.
if git rev-parse HEAD >/dev/null 2>&1; then
    [ -z "$(git status --porcelain -- 'plot/*.hpp.sha512')" ] || fail "a module sidecar drifted — run scripts/sign-modules.sh, commit it, push again"
fi

# 4. Configure + build (debug) -------------------------------------------------------------------
bold "Configuring + building (debug)…"
cmake --preset debug         >/tmp/cheatah_plot_cfg_debug.log   2>&1 || { tail -20 /tmp/cheatah_plot_cfg_debug.log;   fail "configure (debug)"; }
cmake --build --preset debug >/tmp/cheatah_plot_build_debug.log 2>&1 || { tail -30 /tmp/cheatah_plot_build_debug.log; fail "debug build"; }

# 5. System tests: compile each systests/test_*.purr against plot/ and run it ---------------------
...
# plot.scale is GPU-free (cheatah-plot's analog of gpu.dispatch), so unlike cheatah-gpu there is no
# Vulkan SDK to put on CPATH — the systests compile against the pure axis geometry alone.
sfails=0; sran=0
for t in "${tests[@]}"; do
...
#     (extension path + sidecar; no git, no --import-root) — the first-real-consumer contract. ---------
bold "Sandboxing the 'biome add cheatah-plot' user experience…"
...
if [ "${QA_GATE_SKIP_VALGRIND:-0}" = "1" ]; then
    ...
    bash security/run-valgrind.sh >/tmp/cheatah_plot_valgrind.log 2>&1 || { tail -50 /tmp/cheatah_plot_valgrind.log; fail "valgrind memcheck"; }
    tail -1 /tmp/cheatah_plot_valgrind.log
fi
...
TIDY_BUILD_DIR=build/debug TIDY_SOURCE_RE='/(plot|tests)/' bash "$CHEATAH_DIR/scripts/clang_tidy.sh" || fail "clang-tidy"
