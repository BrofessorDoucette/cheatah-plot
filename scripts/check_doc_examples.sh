...
#
# The house convention: every public plot function's docstring carries an `@par Example` with a
# `@code{.purr} … @endcode` block, and each block is a COMPLETE program (its own imports, ready
# to copy-paste). This script extracts every block from plot/*/*.purr and compiles each with
# purrc against this checkout — so an example that drifts from the API breaks the push instead
# of lying on the docs site. (Blocks are compiled, not run: examples may reference files or
# render output; compilation is the truth bar the docs promise.)
#
