#!/usr/bin/env bash
# Javadoc / Doxygen DOCUMENTATION-COVERAGE gate for cheatah-plot's public API.
#
...
#
# The generated submodule headers (plot/<mod>/<mod>.hpp) are EXCLUDED by the Doxyfile: purrc emits a
# little boilerplate (an operator<<, a pretty-printer, aggregate fields) that is not hand-authored API,
# and the header cannot be edited (it must byte-match its `.purr`). Their doc quality lives in the
# reviewed `.purr` source and they are verified end-to-end by the systests. This gate therefore holds the
# hand-written surface (the plot.hpp umbrella and any future hand-written C++) to 100% — the same split
# cheatah-gpu uses to keep its generated Vulkan forwarders out of the strict host doc gate.
#
...
echo "doc-coverage: 100% — every hand-written public plot entity (types, functions, params, returns) is documented."
exit 0
